...
public class WalkingView : MonoBehaviour
{
    [SerializeField] private Animator overworldAnimator;
    // Start is called before the first frame update

    public bool flipRightLeft = false; //GetRidOfThisOnceWeActuallyHaveGraphicsThatWork
    ...
    public void SetDirectionAnim(Vector2 direction)
    {
        this.direction = direction;
        if (direction.x >= velocityThreshold)
        {
            PlayAnimation("walkingRight");
            if (flipRightLeft == true)
            {
